...
def parse_args():
    ...
    p.add_argument("--log_dir", type=str, default="logs/finetune_sync_multiband_reduced_wandb")
    ...
    p.add_argument("--wandb_tags", type=str, nargs="*", default=["multiband-reduced"])
...
def main():
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if wandb:
            # log mix weights mean
            mix_out = torch.softmax(model.sync_filter_out.raw_mix, dim=-1).detach().cpu()
            wandb.log(
                {
                    "val/acc_most_certain": acc,
                    "filters/mix_entropy_mean": float((-(mix_out * (mix_out + 1e-12).log()).sum(-1)).mean()),
                },
                step=global_step,
            )
