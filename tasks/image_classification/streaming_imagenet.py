...
class _StreamingImageNetIterable(IterableDataset):
    ...
    Notes:
    - imagenet-1k is gated; you need HF auth.
    - Prefer setting HF_TOKEN in env (or using `huggingface-cli login` on the machine).
    """
...
@dataclass
class StreamingImageNetLoaders:
...
def get_streaming_imagenet_loaders(
    *,
    n_train: int,
    n_val: int,
    batch_size: int,
    ...
    return StreamingImageNetLoaders(trainloader=trainloader, valloader=valloader)
