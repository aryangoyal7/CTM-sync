...
def _infer_arch_from_state_dict(sd: dict) -> dict:
    ...
    # synapse_depth = (#down_projections blocks) + 1
    down_idxs = []
    for k in sd.keys():
    ...
    # Infer neuron_select_type / n_synch from decay params vs index buffers
...
def parse_args():
    ...
    p.add_argument("--log_dir", type=str, default="logs/colab_finetune_sync_fir")
    ...
    # These are auto-inferred from the checkpoint unless you explicitly override.
...
def main():
    ...
    # Load checkpoint early to infer correct architecture (prevents shape mismatch errors)
    ...
    # Use checkpoint args if present (best), fall back to state_dict inference, then hard defaults.
    ...
    # Initialize lazy modules without depending on ImageNet access yet
